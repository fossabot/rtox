...
def abort(msg):
    ...
    """
    from rtox.fabric.state import output, env
    if not env.colorize_errors:
        red  = lambda x: x
    else:
        from rtox.fabric.colors import red
...
def warn(msg):
    ...
    if not env.colorize_errors:
        ...
        from rtox.fabric.colors import magenta
...
def _pty_size():
    ...
    """
    from rtox.fabric.state import win32
    if not win32:
        import fcntl
        import termios
        import struct
...
def error(message, func=None, exception=None, stdout=None, stderr=None):
    ...
    import rtox.fabric.state
    if func is None:
        func = rtox.fabric.state.env.warn_only and warn or abort
